...
local MP_SIDEBAR = vgui.RegisterTable( PANEL, "DPanel" )
...
function SidebarPresenter:ShowSidebar( mp, spawnMenu )
	...
	local sidebar = vgui.CreateFromTable( MP_SIDEBAR, spawnMenu )
...
hook.Add("OpenMediaPlayerControls", "MP.OpenMediaPlayerControls", function(spawnMenu, theatreScreen)
	if not IsValid(theatreScreen) then return end

	local mp = MediaPlayer.GetByObject( theatreScreen )
	if not IsValid(mp) then return end

	SidebarPresenter:ShowSidebar( mp, spawnMenu )
end)
hook.Add("CloseMediaPlayerControls", "MP.CloseMediaPlayerControls", function()
